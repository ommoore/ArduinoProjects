...
*        monitoring real-time moisture readings. 
*        Modification of original source code.
* @author Elecrow (original)
* @author Oliver Moore (modified by)
* @version 1.0
***********************************************/
#include <Wire.h>
#include "U8glib.h"
...
int moisture_limit_high = 55;  //55% Upper Limit (when to stop watering)
int moisture_limit_low = 30;   //30% Lower Limit (when to start watering)

//component state flags  0:close  1:open   
...
void read_value() {
/************** Use with Resistive Moisture Sensors ***********
  float value1 = analogRead(A0);
  moisture1_value = ((value1 * 120) / 1023); 
  delay(20);
  
  float value2 = analogRead(A1);
  moisture2_value = ((value2 * 120) / 1023); 
  delay(20);
  
  float value3 = analogRead(A2);
  moisture3_value = ((value3 * 120) / 1023); 
  delay(20);
  
  float value4 = analogRead(A3);
  moisture4_value = ((value4 * 120) / 1023); 
  delay(20);
 **********************************************************/
/************ Use with Capacitive Moisture Sensors *********/
  float value1 = analogRead(A0);
...
void water_flower() {
  //Moisture Sensor 1
  if (moisture1_value < moisture_limit_low) {
  ...
  } else if (moisture1_value > moisture_limit_high) {
  ...
  //Moisture Sensor 2
  if (moisture2_value < moisture_limit_low) {
  ...
  } else if (moisture2_value > moisture_limit_high) {
  ...
  //Moisture Sensor 3
  if (moisture3_value < moisture_limit_low) {
  ...
  } else if (moisture3_value > moisture_limit_high) {
  ...
  //Moisture Sensor 4
  if (moisture4_value < moisture_limit_low) {
  ...
  } else if (moisture4_value > moisture_limit_high) {
...
//Style the flowers     
//bitmap_bad: bad flowers   bitmap_good: good flowers
